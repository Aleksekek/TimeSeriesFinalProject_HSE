...
class CatBoostHierarchicalAdapter:
    ...
    def _create_features(self, 
        ...
        if 'rolling_mean_30' in df.columns:
            df['value_to_mean_30'] = df['value'] / (df['rolling_mean_30'] + 1e-8)
        ...
        if self.use_external_features and external_data is not None:
            ...
            if 'oil' in external_data:
                oil_cols = ['oil_price', 'oil_price_lag1', 'oil_price_lag7', 'oil_price_change']
                existing_oil_cols = [c for c in oil_cols if c in external_data['oil'].columns]
            ...
            if 'transactions' in external_data and self.level_name and 'cluster' in self.level_name:
                import re
                ...
                if cluster_match:
                    ...
                    if len(cluster_trans) > 0 and 'transactions' in cluster_trans.columns:
        ...
        categorical_cols = ['year', 'month', 'day', 'dayofweek', 'weekend', 'quarter', 
                            'week_of_year', 'is_holiday', 'holiday_tomorrow', 'holiday_yesterday']
    ...
    def fit(self, 
        ...
        if len(train_features) < 10:  # Минимум 10 строк для обучения
            if self.level_name:
                print(f"    ⚠️  Недостаточно данных для обучения (всего {len(train_features)} строк). Используем fallback.")
            from sklearn.dummy import DummyRegressor
        ...
        categorical_features = ['year', 'month', 'day', 'dayofweek', 'weekend', 'quarter', 
                                'week_of_year', 'is_holiday', 'holiday_tomorrow', 'holiday_yesterday']
        cat_features = []
    ...
    def predict(self, horizon: int, last_data: pd.DataFrame, external_data: Dict = None) -> np.ndarray:
        ...
        if hasattr(self.model, 'predict') and not hasattr(self.model, 'get_best_iteration'):
        ...
        if self.level_name:
            print(f"    Прогнозирование для {self.level_name}, признаков: {len(available_features)}")
                ...
                for f in available_features:
                    if f in last_row.columns:
                        ...
                        if self.level_name and step == 0:
                            print(f"      Предупреждение: признак {f} отсутствует в данных")
